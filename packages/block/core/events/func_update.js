 ...
 * @param {Object} oldMutation The mutation of function.
 * @param {Object} newMutation The new mutation of function.
 * @extends {FuncBase}
 * @constructor
 */
export const FuncUpdate = function(workspace, oldMutation, newMutation) {
  ...
  this.oldMutation = JSON.stringify(oldMutation);
  this.newMutation = JSON.stringify(newMutation);
};
...
FuncUpdate.prototype.run = function(forward) {
  ...
  if (forward) {
    ...
    const newMutationObj = JSON.parse(this.newMutation);
    const newProcCode = newMutationObj['proccode'];
