 ...
 * @constructor
 */
export const Connection = function(source, type) {
  /**
   * @type {!Blockly.Block}
   * @protected
   */
 ...
 */
Connection.prototype.isConnectedToNonInsertionMarker = function() {
 ...
 * @private
 */
Connection.prototype.canConnectWithReason_ = function(target) {
  if (!target) {
  ...
  if (blockA && blockA == blockB) {
    ...
    return Connection.REASON_SHADOW_PARENT;
      ...
      (blockA.type != constants.PROCEDURES_PROTOTYPE_BLOCK_TYPE &&
      scratchBlocksUtils.isShadowArgumentReporter(blockB))) {
    // cc: At present, the argument reporter should not appear in places other
    // than prototype. This may prevent parameter passing blocks from working
    // properly in the future. Maybe we need a more elegant solution?
 ...
 * @private
 */
Connection.prototype.checkConnection_ = function(target) {
 ...
 */
Connection.prototype.canConnectToPrevious_ = function(candidate) {
 ...
 * @private
 */
Connection.singleConnection_ = function(block, orphanBlock) {
 ...
 */
Connection.prototype.getOutputShape = function() {
 ...
 * @private
 */
Connection.prototype.neighbours_ = function(/* maxLimit */) {
  return [];
};
