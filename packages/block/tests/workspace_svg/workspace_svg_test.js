...
function helper_createWorkspaceWithToolbox() {
  ...
  return Blockly.inject.inject('blocklyDiv', {toolbox: toolbox});
}
...
function test_appendDomToWorkspace() {
  ...
  try {
    ...
    assertEquals('Block 2 position y',23 + blocks[0].getHeightWidth().height + Blockly.renderer.constants.SEP_SPACE_Y, blocks[1].getRelativeToSurfaceXY().y);
  } finally {
    workspace.dispose();
  }
}
