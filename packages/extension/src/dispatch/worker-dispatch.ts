...
class _WorkerDispatch extends SharedDispatch {
    ...
    constructor () {
        ...
        if (typeof self !== 'undefined') {
            self.onmessage = this._onMessage.bind(this, self as unknown as Worker);
        }
    }

    /**
     * @returns {Promise<void>} a promise which will resolve upon connection to central dispatch. If you need to make a call
     ...
     *      })
     */
    get waitForConnection (): Promise<void> {
    ...
    setService (service: string, provider: unknown): Promise<void> {
        ...
        return this.waitForConnection.then(
            () => {
                this._remoteCall(self as unknown as Worker, 'dispatch', 'setService', service);
            }
        );
    }
     ...
     * @protected
     */
    protected _getServiceProvider (service: string): { provider: unknown; isRemote: boolean } {
     ...
     * @protected
     */
    protected _onDispatchMessage (worker: Worker, message: DispatchCallMessage): Promise<void> | undefined {
        let promise: Promise<void> | undefined;
        switch (message.method) {
        case 'handshake':
            if (this._onConnect) {
                promise = Promise.resolve(this._onConnect());
            }
            break;
        case 'terminate':
