...
test('importing an sb2 project with cloud variables', done => {
    ...
    vm.loadProject(cloudVarSimple).then(() => {
        ...
        expect(Number(variable.value)).toBe(100); // Though scratch 2 requires
...
test('importing an sb2 project with cloud variables at the limit for a project', done => {
    const vm = new VirtualMachine();
    vm.attachStorage(makeTestStorage());
...
test('importing an sb2 project with cloud variables exceeding the limit for a project', done => {
