...
interface GetTranslateArgs {
    WORDS: string;
    LANGUAGE: string;
}

type LanguageKeys = keyof typeof languageNames.menuMap;
type LanguageNames = keyof typeof languageNames.nameMap;
type ScratchToGoogleKeys = keyof typeof languageNames.scratchToGoogleMap;
...
class Scratch3TranslateBlocks implements ExtensionClass {
     ...
     */
    private _viewerLanguageCode = this.getViewerLanguageCode();
     ...
     */
    private _supportedLanguages: ExtensionMenuItemObject[] = [];
     ...
     */
    private _randomLanguageCode = 'en';
    ...
    static get STATE_KEY (): string {
        return 'Scratch.translate' as const;
    }
     ...
     * @param code The language code to get the list of language pairs
     * @returns An array of languge name and language code pairs.
     */
    private _getSupportedLanguages (code: LanguageKeys): ExtensionMenuItemObject[] {
     ...
     * @returns the language name of the project viewer.
     */
    getViewerLanguage (): string {
        ...
        if (!langNameObj && languageNames.scratchToGoogleMap[this._viewerLanguageCode as ScratchToGoogleKeys]) {
            const lookupCode = languageNames.scratchToGoogleMap[this._viewerLanguageCode as ScratchToGoogleKeys];
            langNameObj = names.find(obj => obj.code === lookupCode);
        }

        let langName: string = this._viewerLanguageCode;
        if (langNameObj) {
    ...
    getViewerLanguageCode () {
        const locale = formatMessage.setup().locale as string;
    ...
    getLanguageCodeFromArg (arg: unknown): string {
        ...
        if (Object.prototype.hasOwnProperty.call(languageNames.nameMap, languageArg)) {
            return languageNames.nameMap[languageArg as LanguageNames];
        }
    ...
    getTranslate (args: GetTranslateArgs) {
        ...
        let urlBase = `${this.runtime.translateServiceUrl}?language=`;
        urlBase += lang;
        urlBase += '&text=';
