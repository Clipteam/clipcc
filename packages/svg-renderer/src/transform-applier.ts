...
const _transformPath = function (pathString: string, transform: Matrix.Matrix) {
    ...
    const getCoord = function (index: number, coord: keyof PointObjectNotation) {
    ...
    const getPoint = function (index: number) {
        return {x: getCoord(index, 'x'), y: getCoord(index + 1, 'y')};
    };

    const roundTo4Places = function (num: number) {
        return Number(num.toFixed(4));
    };

    // Returns the transformed point as a string
    const getString = function (point: PointObjectNotation) {
...
const _isContainerElement = function (element: Element) {
...
const _quadraticMean = function (a: number, b: number) {
...
const _createGradient = function (gradientId: string, svgTag: SVGElement, bbox: BBox, matrix: Matrix.Matrix) {
    ...
    if (!defs) {
        defs = SvgElement.create('defs') as SVGDefsElement;
    ...
    if (radial) {
        ...
        radius = Number(getValue(newGradient, 'r', false, false, scaleToBounds, '50%'));
...
const _parseUrl = (value: string | undefined, windowRef: Window) => {
...
const transformStrokeWidths = function (svgTag: SVGElement, windowRef: Window, bboxForTesting?: BBox) {
    ...
    const applyTransforms = (element: Element, matrix: Matrix.Matrix, strokeWidth: number, fill?: string, stroke?: string) => {
        if (_isContainerElement(element)) {
            ...
            if (fillGradientId || strokeGradientId) {
                ...
                if (bboxForTesting) {
                    ...
                    try {
                        ...
                        const svg = SvgElement.set(doc.createElementNS(SvgElement.svg, 'svg') as SVGElement);
                        const path = SvgElement.set(doc.createElementNS(SvgElement.svg, 'path') as SVGElement);
                        // @ts-expect-error ignore it
            ...
            const matrixScale = _getScaleFactor(matrix);
            element.setAttribute('stroke-width', String(_quadraticMean(matrixScale.x, matrixScale.y) * strokeWidth));
            if (fill) element.setAttribute('fill', fill);
            if (stroke) element.setAttribute('stroke', stroke);
...
export default transformStrokeWidths;
